...
function renderFilters(){
  ...
  $('sectionSelect').innerHTML = sections
    .map(s=>`<option value="${esc(s)}">${esc(SECTION_LABELS[s] || s)}</option>`)
    .join('');

  const months = ['ALL', ...[...new Set(state.rows.map(r=>r.Month))].sort((a,b)=>new Date(a)-new Date(b))];
  $('monthSelect').innerHTML = months
    .map(m=>`<option value="${esc(m)}">${esc(m === 'ALL' ? 'All months' : m)}</option>`)
    .join('');
...
function renderReport(){
  ...
  if(view === 'summary'){
    ...
    if(!rows.length){
      $('reportTable').innerHTML = '<div class="empty">No summary rows for the selected filter.</div>';
      return;
    }
    ...
    rows.forEach(r=>{
      html += '<tr>' +
        `<td>${esc(r.Month)}</td>` +
        `<td>${esc(SECTION_LABELS[r.Section] || r.Section)}</td>` +
        `<td>${r.Bills}</td>` +
        `<td>${fmt(r.Taxable)}</td>` +
        `<td>${fmt(r.IGST)}</td>` +
        `<td>${fmt(r.CGST)}</td>` +
  ...
  if(!rows.length){
    $('reportTable').innerHTML = '<div class="empty">No bill rows for the selected filter.</div>';
    return;
  }
...
function excelData(){
  if($('viewSelect').value === 'summary'){
    return getSummary().map(r=>({
      ...
      bills:r.Bills,
      total_taxable_value:r.Taxable,
      total_igst:r.IGST,
      total_cgst:r.CGST,
      total_sgst:r.SGST,
      total_cess:r.CESS,
      total_invoice_value:r.Total
    }));
  }

  return getBills().map(r=>({
    month:r.Month,
    section:SECTION_LABELS[r.Section] || r.Section,
    date:r.Date,
    party:r.Party,
...
function buildNarration(igstInfo, cgstInfo, sgstInfo){
  ...
  if (igstInfo && igstInfo.amount > 0.001){
    parts.push(`IGST ${formatRateLabel(igstInfo.rate ?? round2(igstInfo.rawRate))}%`);
  }
  if (cgstInfo && cgstInfo.amount > 0.001){
    parts.push(`CGST ${formatRateLabel(cgstInfo.rate ?? round2(cgstInfo.rawRate))}%`);
  }
  if (sgstInfo && sgstInfo.amount > 0.001){
    parts.push(`SGST ${formatRateLabel(sgstInfo.rate ?? round2(sgstInfo.rawRate))}%`);
  }
...
function getAdjustmentLedgerName(value){
  return Math.abs(Number(value || 0)) > 10 ? 'Input Adjustment alc' : 'Round Off alc';
}
...
function generateXML(){
  ...
  rows.forEach(r=>{
    ...
    if(r.Party && !uniquePartyLedgers.has(r.Party)){
      uniquePartyLedgers.set(r.Party, gstin);
    }
  ...
  [...taxLedgers].forEach(name=>{
    xml += addLedgerXml(name, 'Duties & Taxes');
  });
  ...
  rows.forEach(r=>{
    const section = clean(r.Section).toUpperCase();
    const docType = clean(r.DocType).toUpperCase();
    ...
    const taxable = round2(r.Taxable || 0);
    let igst = round2(r.IGST || 0);
    let cgst = round2(r.CGST || 0);
    let sgst = round2(r.SGST || 0);
    ...
    const referenceDate = /^[0-9]{8}$/.test(String(r.TallyDate || '')) ? String(r.TallyDate) : lastDay(r.Period).tally;
    const supplierInvoiceNo = clean(r.Invoice || '');
    const totalStr = total.toFixed(2);
    const taxableStr = taxable.toFixed(2);

    xml += `<TALLYMESSAGE xmlns:UDF="TallyUDF">\n`;
    xml += `<VOUCHER VCHTYPE="${esc(voucherType)}" ACTION="Create" OBJVIEW="Accounting Voucher View">\n`;
    xml += `<DATE>${voucherDate}</DATE>\n`;
    xml += `<VOUCHERTYPENAME>${esc(voucherType)}</VOUCHERTYPENAME>\n`;
    xml += `<VOUCHERNUMBER>${esc(r.Invoice)}</VOUCHERNUMBER>\n`;
    xml += `<REFERENCE>${esc(supplierInvoiceNo)}</REFERENCE>\n`;
    xml += `<REFERENCEDATE>${referenceDate}</REFERENCEDATE>\n`;
    xml += `<PERSISTEDVIEW>Accounting Voucher View</PERSISTEDVIEW>\n`;
    xml += `<ISINVOICE>No</ISINVOICE>\n`;
    xml += `<PARTYLEDGERNAME>${esc(r.Party)}</PARTYLEDGERNAME>\n`;
    xml += `<NARRATION>${esc(narration)}</NARRATION>\n`;

    if(isCreditNote){
      xml += `<LEDGERENTRIES.LIST>\n`;
      xml += `<LEDGERNAME>${esc(r.Party)}</LEDGERNAME>\n`;
      xml += `<ISDEEMEDPOSITIVE>No</ISDEEMEDPOSITIVE>\n`;
      xml += `<ISPARTYLEDGER>Yes</ISPARTYLEDGER>\n`;
      xml += `<ISLASTDEEMEDPOSITIVE>No</ISLASTDEEMEDPOSITIVE>\n`;
      xml += `<AMOUNT>${totalStr}</AMOUNT>\n`;
      ...
      xml += `<LEDGERENTRIES.LIST>\n`;
      xml += `<LEDGERNAME>${esc(purchaseLedger)}</LEDGERNAME>\n`;
      xml += `<ISDEEMEDPOSITIVE>No</ISDEEMEDPOSITIVE>\n`;
      xml += `<AMOUNT>-${taxableStr}</AMOUNT>\n`;
      xml += `</LEDGERENTRIES.LIST>\n`;
      ...
      if (Math.abs(initialBalanceAdjustment) > 0.001){
        const adjustmentLedger = getAdjustmentLedgerName(initialBalanceAdjustment);
        xml += `<LEDGERENTRIES.LIST>\n<LEDGERNAME>${esc(adjustmentLedger)}</LEDGERNAME>\n<ISDEEMEDPOSITIVE>${initialBalanceAdjustment > 0 ? 'Yes' : 'No'}</ISDEEMEDPOSITIVE>\n<AMOUNT>${(-initialBalanceAdjustment).toFixed(2)}</AMOUNT>\n</LEDGERENTRIES.LIST>\n`;
      }

      if (Math.abs(finalBalanceAdjustment) > 0.001){
        const adjustmentLedger = getAdjustmentLedgerName(finalBalanceAdjustment);
...
async function processFiles(){
  ...
  notes.push(`Processed ${files.length} file(s)`);
  notes.push(`Financial year: ${state.fy || 'Unknown'}`);
  notes.push(`Rows created: ${state.rows.length}`);
  if(state.unknownKeys.size){
    notes.push(`Miscellaneous keys: ${[...state.unknownKeys].join(', ')}`);
  }
...
function renderFileChips(){
  ...
  $('fileChips').innerHTML = files.length
    ? files.map(f=>`<span class="chip">${esc(f.name)}</span>`).join('')
...
function bind(){
  ...
  $('downloadExcelBtn').addEventListener('click', ()=>{
    ...
    if(!data.length){
      alert('No rows for current selection.');
      return;
    }
    ...
    dl(
      `gstr-2b-${$('sectionSelect').value.toLowerCase()}-${$('viewSelect').value}.xlsx`,
      new Blob([arrData], { type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' }),
  ...
  $('downloadXmlBtn').addEventListener('click', ()=>{
    ...
    if(!xml){
      alert('No bill rows available for current selection.');
      return;
    }
    ...
    const name = `gstr-2b-tally-${sec.toLowerCase()}-${safeMonth}.xml`;

    state.lastXML = xml;
    dl(name, xml, 'application/xml;charset=utf-8');
  });

  ['sectionSelect','viewSelect','monthSelect'].forEach(id=>{
  ...
  dz.addEventListener('drop', async e=>{
    ...
    if(files && files.length){
      try{
        ...
        console.warn('Could not assign dropped files to input directly:', err);
      }
